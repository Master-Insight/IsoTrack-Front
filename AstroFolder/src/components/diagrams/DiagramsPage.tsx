...
export default function DiagramsPage({ diagramsEndpoint, linksEndpoint }: DiagramsPageProps) {
  ...
  const buildSvgExport = () => {
    ...
    const svgEdges = edges
      .map((edge) => {
        ...
        const x2 = target.x + 70;
        const y2 = target.y + 22;
        const label = edge.label ? `<text x="${(x1 + x2) / 2}" y="${(y1 + y2) / 2 - 8}" font-size="11" fill="#475569" text-anchor="middle">${edge.label}</text>` : "";
                ...
                <button
                  ...
                  className={`w-full text-left px-5 py-4 transition-colors ${
                    diagram.id === selectedId
                      ? "bg-primary/5 border-l-4 border-primary font-semibold text-primary"
                      : "hover:bg-primary/5"
                  }`}
          ...
          <div className="card-surface border border-slate-200 shadow-lg">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
              ...
              <div className="flex gap-2">
                <button className="button-secondary" type="button" onClick={() => handleAddNode("area")}>Añadir área</button>
                <button className="button-secondary" type="button" onClick={() => handleAddNode("role")}>Añadir rol</button>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                    ...
                    <p className="break-all text-[11px]">{selectedDiagram?.svg_export ? selectedDiagram.svg_export.slice(0, 120) + "..." : "Sin exportar"}</p>
                  </div>
                </div>
              </div>
            </div>
